...
export function ListingCard2({ listing }) {
    ...
    <Card className="bg-gray-900 border-gray-800 hover:border-gray-700 transition-all duration-200 group">
      <CardContent className="p-0">
        ...
        <div className="relative aspect-square overflow-hidden rounded-t-lg">
          <img
            src={`/placeholder.svg?height=300&width=300&query=${encodeURIComponent(listing.title)}`}
            alt={listing.title}
          ...
          <div className="absolute top-2 left-2">
            <Badge className={`${getTypeColor(listing.type)} text-white text-xs`}>{listing.category}</Badge>
          </div>
        ...
        <div className="p-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              <span className="text-green-400 font-bold text-xl">{formatPrice(listing.price_usd)}</span>
              {listing.shipping && (
